...
from mctot.core.state import State


class SudokuChecker:
    """Validity / completeness checker for an n x n sudoku.

    Validity rules (matching the upstream solver): correct size, every filled
    cell holds a number in ``1..n``, no row or column duplicates (blocks are
    NOT checked), blanks must be ``"*"``, and a previously-filled cell cannot
    be changed to a different number.
    """
            ...
            for c in range(self.puzzle_size):
                ...
                try:
                    ...
                    if not (1 <= num <= self.puzzle_size):
                        return (
                            False,
                            f"Cell [{r}][{c}] is filled with {num}, which is not in the range of 1 to {self.puzzle_size} for {self.d} sudoku.",
                        )
                    if (
                        prev_board
                        and prev_board[r][c] != "*"
                        and prev_board[r][c] != cell
                    ):
                        # for example, prev_board = 1 -> cell = 2
                        return (
                            False,
                            f"Cell [{r}][{c}] has been filled with {prev_board[r][c]} previously. We cannot set it to a diffent number.",
                        )
                    ...
                    if num in col_sets[c]:
                        return (
                            False,
                            f"There is duplicate number {num} in column {c + 1}.",
                        )
                    col_sets[c].add(num)
                except ValueError:
                    # not a number
                    if cell != "*":
                        return (
                            False,
                            f"The empty cell should be filled with '*' rather than {cell}.",
                        )
                    continue
